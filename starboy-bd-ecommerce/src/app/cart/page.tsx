        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
            <div className="lg:col-span-2 space-y-3">
              <AnimatePresence>
                {items.map((item) => (
                  <motion.div key={item.product.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="flex gap-4 bg-white rounded-2xl p-4 border border-ink-100 shadow-soft">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-2 border border-ink-200 rounded-lg px-1.5 py-1">
                          <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="p-1 hover:bg-ink-100 rounded"><Minus className="w-3.5 h-3.5" /></button>
                          <span className="text-sm font-semibold w-5 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-1 hover:bg-ink-100 rounded"><Plus className="w-3.5 h-3.5" /></button>
                        </div>
                        <div className="flex items-center gap-4">
                          ...
                          <button onClick={() => removeFromCart(item.product.id)} className="p-2 text-ink-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </div>
                    </div>
                  </motion.div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl p-6 border border-ink-100 shadow-soft sticky top-24">
                ...
                {/* Req 5: Message Seller on FB instead of Checkout */}
                <a href={FACEBOOK_URL} target="_blank" rel="noopener noreferrer" className="w-full mt-6 flex items-center justify-center gap-2 py-3.5 bg-[#1877F2] hover:bg-[#0f63d8] text-white text-sm font-bold rounded-xl transition-colors">
                  <MessageCircle className="w-4 h-4" /> Message Seller in FB About This
                </a>
                <p className="text-center text-[11px] text-ink-400 mt-3">Seller will reply with payment details</p>
              </div>
            </div>
          </div>
        )}
