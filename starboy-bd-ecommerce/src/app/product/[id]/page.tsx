...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const { data: p } = await firebaseData.from("products").select("*").eq("slug", slug).single();
        if (cancelled) return;
        if (p) {
          setProduct(p);
          const [{ data: r }, { data: rel }] = await Promise.all([
            firebaseData.from("reviews").select("*").eq("productId", p.id),
            firebaseData.from("products").select("*").neq("slug", slug).limit(4),
          ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const result = await firebaseData.from("messages").insert({
        userId: user.id,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-6">
              ...
              {messageOpen && (
                <div className="mt-3 bg-white border border-ink-100 rounded-2xl p-4 shadow-soft">
                  <textarea
                    value={messageText}
                    onChange={(e) => setMessageText(e.target.value)}
                    rows={4}
                    placeholder={`Ask about ${product.name}...`}
                    className="w-full px-4 py-3 bg-ink-50 border border-ink-200 rounded-xl text-sm outline-none focus:border-brand-500 resize-none"
                  />
                  <div className="flex justify-end mt-3">
                    <button
                      onClick={handleSendMessage}
                      disabled={messageSending}
                      className="inline-flex items-center gap-2 px-5 py-2.5 bg-ink-950 text-brand-300 text-sm font-semibold rounded-xl hover:bg-brand-700 hover:text-white transition-colors disabled:opacity-60"
                    >
