...
export default function ProductPage() {
  ...
  useEffect(() => {
    if (product) {
      ...
      firebaseData.from("products").update({ viewCount: currentCount + 1 }).eq("id", product.id).catch(() => {});
    }
  }, [product?.id]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        // Resolve by slug first; fall back to document id so links that use the
        // product id (e.g. from testimonials) still work.
        let { data: p } = await firebaseData.from("products").select("*").eq("slug", slug).single();
        if (!p) {
          const byId = await firebaseData.from("products").select("*").eq("id", slug).single();
          p = byId.data;
        }
        if (cancelled) return;
        if (p) {
          ...
          const [{ data: r }, { data: rel }] = await Promise.all([
            // Limit reviews so a popular product doesn't read hundreds of docs.
            firebaseData.from("reviews").select("*").eq("productId", p.id).order("createdAt", { ascending: false }).limit(20),
            // Bounded related-products query (server-side limit) instead of a full catalog scan.
            firebaseData.from("products").select("*").order("createdAt", { ascending: false }).limit(5),
          ]);
          if (!cancelled) {
            setReviews((r as Review[]) || []);
            // Drop the current product if it appears in the related list, cap at 4.
            setRelated(((rel as Product[]) || []).filter((x) => x.slug !== slug && x.id !== p.id).slice(0, 4));
          }
        }
