...
import { initializeFirebaseDatabase, resetFirebaseDatabase } from "@/lib/firestoreSeed";
...
export default function AdminPage() {
  ...
  const loadAll = async () => {
    ...
    const [p, c, a, m, s, sets] = await Promise.all([
      ...
      firebaseData.from("messages").select("*").order("createdAt", { ascending: false }),
    ...
    if (p.error || c.error) {
      ...
      setDbReady(Boolean((p.data?.length || 0) > 0 && (c.data?.length || 0) > 0));
    }
  ...
  const runSetup = async () => {
    ...
    try {
      ...
      setDbReady(true);
      await loadAll();
      alert("Firebase Firestore database initialized successfully.");
    } catch (err: any) {
      alert(err?.message || "Failed to initialize Firebase Firestore database");
    } finally {
  ...
  const runReset = async () => {
    const confirmed = confirm(
      "Reset Firestore database? This will delete and recreate products, categories, settings, stats, announcements, carts, wishlists, orders, messages, and reviews. User accounts will NOT be deleted."
    );
    ...
    try {
      ...
      setDbReady(true);
      await loadAll();
      alert("Firebase Firestore database reset successfully.");
    } catch (err: any) {
      alert(err?.message || "Failed to reset Firebase Firestore database");
    } finally {
  ...
  const saveProduct = async () => {
    ...
    if (editingId) {
      await firebaseData.from("products").update(payload).eq("id", editingId);
    } else {
  ...
  const saveCategory = async () => {
    ...
    if (editingId) await firebaseData.from("categories").update(payload).eq("id", editingId);
    else await firebaseData.from("categories").insert(payload);
    setModalOpen(false); loadAll();
  };
  const deleteCategory = async (id: string) => { if (!confirm("Delete?")) return; await firebaseData.from("categories").delete().eq("id", id); loadAll(); };
  ...
  const saveAnn = async () => {
    ...
    if (editingId) await firebaseData.from("announcements").update(payload).eq("id", editingId);
    else await firebaseData.from("announcements").insert(payload);
    setModalOpen(false); loadAll();
  };
  const deleteAnn = async (id: string) => { if (!confirm("Delete?")) return; await firebaseData.from("announcements").delete().eq("id", id); loadAll(); };
  ...
  const saveStat = async () => {
    ...
    if (editingId) await firebaseData.from("stats").update(payload).eq("id", editingId);
    else await firebaseData.from("stats").insert(payload);
    setModalOpen(false); loadAll();
  };
  const deleteStat = async (id: string) => { if (!confirm("Delete stat?")) return; await firebaseData.from("stats").delete().eq("id", id); loadAll(); };

  // --- Setting inline edit ---
  const saveSetting = async (key: string, value: string) => {
    if (!db) {
      toast.error("Firebase Firestore is not initialized");
      return;
    }
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {!dbReady && (
              <button
                onClick={runSetup}
                disabled={setupRunning || resetRunning}
                className="flex items-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-500 text-white text-xs font-bold rounded-xl transition-colors disabled:opacity-60"
              >
                {setupRunning ? <Loader2 className="w-4 h-4 animate-spin" /> : <Database className="w-4 h-4" />}
                {setupRunning ? "Initializing..." : "Initialize Firestore Database"}
              </button>
            )}
            ...
            <button
              onClick={runReset}
              disabled={setupRunning || resetRunning}
              className="flex items-center gap-2 px-5 py-2.5 bg-ink-900 hover:bg-red-700 text-red-300 hover:text-white border border-red-900/40 text-xs font-bold rounded-xl transition-colors disabled:opacity-60"
            >
              {resetRunning ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
              {resetRunning ? "Resetting..." : "Reset Firestore Database"}
            </button>
          </div>
        </div>
                      ...
                      <div key={set.id} className="flex items-center gap-3 justify-between">
                        <div className="flex-1">
                          ...
                          {editSettingKey === set.key ? (
                            ...
                            <div className="text-sm text-ink-200 mt-0.5">{String(set.value ?? "")}</div>
                          )}
                        </div>
                        <button
                          onClick={() => {
                            setEditSettingKey(set.key);
                            setEditSettingValue(String(set.value ?? ""));
                          }}
