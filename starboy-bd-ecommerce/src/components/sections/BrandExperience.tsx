    ...
    const loadSettings = async () => {
      try {
        ...
        if (!cancelled && Array.isArray(data)) {
          ...
          data.forEach((item: any) => { if (item?.key) map[item.key] = String(item.value ?? ""); });
          setSettings(map);
        }
      } catch (error) { console.error("Failed to load brand experience settings", error); }
    };
    loadSettings();
    ...
    <section className="py-16 md:py-24 bg-ink-950">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="text-center mb-10 md:mb-14">
          ...
          <Link href={FACEBOOK_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-5 px-5 py-2.5 bg-[#1877F2] hover:bg-[#0f63d8] text-white text-sm font-semibold rounded-xl transition-colors">
