...
export default function ProductCard({ product, index = 0 }: { product: Product; index?: number }) {
  ...
  const { isInWishlist, addToWishlist, removeFromWishlist, totalItems: wishCount } = useWishlist();
  const { user } = useAuth();
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: index * 0.05 }} className="group relative">
      <Link href={`/product/${product.slug}`} className="block">
        <div className="relative aspect-[3/4] bg-ink-50 rounded-2xl overflow-hidden mb-3">
          <Image src={product.images[0]} alt={product.name} fill className="object-cover transition-transform duration-300 group-hover:scale-105" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" loading={index < 4 ? "eager" : "lazy"} quality={75} />
          ...
          {product.trending && <div className="absolute top-3 right-3 bg-ink-950/80 text-brand-300 text-[10px] font-bold px-2.5 py-1 rounded-full">Trending</div>}
          {isOutOfStock && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <span className="bg-ink-950 text-white text-xs font-bold px-4 py-2 rounded-full">Out of Stock</span>
            </div>
          )}
          <div className="absolute inset-x-0 bottom-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-200">
            <div className="flex gap-2">
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleAddToCart(); }} disabled={isOutOfStock} className={`flex-1 flex items-center justify-center gap-2 text-xs font-semibold py-2.5 rounded-xl shadow-soft transition-colors ${isOutOfStock ? 'bg-ink-300 text-white cursor-not-allowed' : 'bg-white text-ink-900 hover:bg-brand-700 hover:text-white'}`}>
                <ShoppingBag className="w-3.5 h-3.5" /> {isOutOfStock ? "Out of Stock" : "Add to Cart"}
              </button>
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleWishlist(); }} className={`p-2.5 rounded-xl shadow-soft transition-colors ${inWishlist ? "bg-red-500 text-white" : "bg-white text-ink-700 hover:bg-red-50 hover:text-red-500"}`}>
        ...
        <div className="space-y-1">
          <h3 className="text-sm font-semibold text-ink-900 group-hover:text-brand-700 transition-colors">{product.name}</h3>
