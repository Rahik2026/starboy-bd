...
export default function AnnouncementBar() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await firebaseData.from("announcements").select("*").eq("active", true).eq("type", "bar").order("priority").limit(1);
  ...
  return (
    <div className="bg-ink-950 text-brand-300 text-xs md:text-sm font-medium tracking-wide relative z-50 animate-fade-in">
